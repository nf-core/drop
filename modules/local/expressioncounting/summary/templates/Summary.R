...
coverage_dt <- merge(data.table(RNA_ID = colnames(ods), read_count = colSums(cnts_mtx),
    isExternal = ods@colData\$isExternal), bam_coverage, by = "RNA_ID", sort = FALSE)
# read counts
...
# Show this plot only if there are external samples, as the next plot contains
# the same info
if (has_external == T) {
...
if (has_external) {
    filter_mtx <- list(local = cnts_mtx_local, all = cnts_mtx, `passed FPKM` = cnts_mtx[rowData(ods)\$passedFilter,
    ...
    filter_dt <- lapply(names(filter_mtx), function(filter_name) {
        ...
        rbindlist
    filter_dt[, filter := factor(filter, levels = c("local", "all", "passed FPKM",
        "min 1 read", "min 10 reads"))]
} else {
    filter_mtx <- list(all = cnts_mtx, `passed FPKM` = cnts_mtx[rowData(ods)\$passedFilter,
        ], `min 1 read` = cnts_mtx[rowQuantiles(cnts_mtx, probs = quant) > 1, ],
        `min 10 reads` = cnts_mtx[rowQuantiles(cnts_mtx, probs = quant) > 10, ])
    filter_dt <- lapply(names(filter_mtx), function(filter_name) {
        mtx <- filter_mtx[[filter_name]]
        ...
        rbindlist
    filter_dt[, filter := factor(filter, levels = c("all", "passed FPKM", "min 1 read",
        "min 10 reads"))]
}

binwidth <- 0.2
p_hist <- ggplot(filter_dt, aes(x = median_counts, fill = filter)) + geom_histogram(binwidth = binwidth) +
    scale_x_log10() + facet_wrap(. ~ filter) + labs(x = "Mean counts per gene", y = "Frequency",
    title = "Mean Count Distribution") + guides(col = guide_legend(title = NULL)) +
    scale_fill_brewer(palette = "Paired") + theme_cowplot() + theme(legend.position = "none")

p_dens <- ggplot(filter_dt, aes(x = median_counts, col = filter)) + geom_density(aes(y = binwidth *
    ..count..), size = 1.2) + scale_x_log10() + labs(x = "Mean counts per gene",
    y = "Frequency") + guides(col = guide_legend(title = NULL)) + scale_color_brewer(palette = "Paired") +
...
if (isEmpty(sex_idx)) {
    ...
    if (isEmpty(xist_idx) | isEmpty(uty_idx)) {
        ...
        sex_dt <- data.table(sampleID = colnames(ods), XIST = counts(ods)[xist_idx,
            ], UTY = counts(ods)[uty_idx, ])
        sex_dt <- merge(sex_dt, as.data.table(colData(ods))[, c(1, sex_idx), with = F],
            sort = F)
...
writeLines(c("\"${task.process}\":", paste("    r-base:", strsplit(version[["version.string"]],
    ...
    paste("    r-tidyr:", as.character(packageVersion("tidyr"))), paste("    bioconductor-GenomicAlignments::",
        as.character(packageVersion("GenomicAlignments"))), paste("    bioconductor-summarizedexperiment:",
