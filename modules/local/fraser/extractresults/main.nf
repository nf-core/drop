process FRASER_EXTRACTRESULTS {
    ...
    input:
    tuple val(meta), path(fds, stageAs: 'savedObjects/*'), path(txdb), val(drop_group), val(annotation_id), val(sample_ids)
    ...
    script:
    template '08_extract_results_FraseR.R'
